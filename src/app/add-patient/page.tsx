...
// Update FormErrors to include general errors
...
async function handleFormSubmission(prevState: FormState, formData: FormData): Promise<FormState> {
  // Validate all fields
  ...
  try {
    ...
    return {
      ...prevState,
      formData,
      errors: { _form: 'Failed to register patient. Please try again.' },
      success: false,
...
export default function PatientRegistrationForm() {
  ...
  const [localFormData, setLocalFormData] = useState<FormData>(initialState.formData);

  // Use refs to track which field is currently focused
  const focusedFieldRef = useRef<string | null>(null);

  // Sync local form data with server state when submission is successful
  useEffect(() => {
    if (state.success) {
      setLocalFormData(state.formData);
    }
  }, [state.success, state.formData]);
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (touched[fieldName]) {
      ...
      // We could set local errors here if needed
    }
  };

  const handleFocus = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name } = e.target;
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setLocalFormData(prev => ({ ...prev, document: file }));
  };

  const handleCancel = () => {
    setLocalFormData(initialState.formData);
    setTouched({});
  };

  // Use localFormData for all form display - this prevents cursor jumping
  const displayFormData = localFormData;
  ...
  const InputField = ({ label, name, type = 'text', required = false, disabled = false }: InputFieldProps) => (
    <div className="relative">
      <input
        ...
        name={name}
        id={name}
        value={localFormData[name] as string}
        onChange={handleChange}
        onFocus={handleFocus}
        onBlur={handleBlur}
        disabled={disabled || isPending}
        className={`peer w-full px-4 pt-6 pb-2 border rounded-lg outline-none transition-all ${state.errors[name] && touched[name]
          ? 'border-red-500 focus:border-red-600'
          : 'border-black focus:border-blue-500'
          } ${disabled ? 'bg-gray-100' : 'bg-white'} ${isPending ? 'opacity-50 cursor-not-allowed' : ''}`}
        placeholder=" "
      />
      <label
        htmlFor={name}
        className={`absolute left-4 transition-all duration-200 pointer-events-none ${displayFormData[name] || type === 'date'
  ...
  const SelectField = ({ label, name, options, required = false }: SelectFieldProps) => (
    <div className="relative">
      <select
        name={name}
        id={name}
        value={displayFormData[name] as string}
        onChange={handleChange}
        onFocus={handleFocus}
        onBlur={handleBlur}
        disabled={isPending}
        className={`peer w-full px-4 pt-6 pb-2 border rounded-lg outline-none transition-all appearance-none ${state.errors[name] && touched[name]
          ? 'border-red-500 focus:border-red-600'
          : 'border-black focus:border-blue-500'
          } ${!displayFormData[name] ? 'text-gray-500' : 'text-gray-900'} ${isPending ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
        <option value=""></option>
        {options.map(opt => (
      ...
      <label
        htmlFor={name}
        className={`absolute left-4 transition-all duration-200 pointer-events-none ${displayFormData[name]
  ...
  const TextAreaField = ({ label, name, required = false }: TextAreaFieldProps) => (
    <div className="relative">
      <textarea
        ...
        disabled={isPending}
        rows={3}
        className={`peer w-full px-4 pt-6 pb-2 border rounded-lg outline-none transition-all resize-none ${state.errors[name] && touched[name]
          ? 'border-red-500 focus:border-red-600'
          : 'border-black focus:border-blue-500'
    ...
    <div>
      ...
      <div className="min-h-screen bg-[#ECF0F9] px-4 sm:px-6 lg:px-8 py-3">
        <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
          ...
          <form action={() => formAction(localFormData)}>
            <div className="space-y-8">
              ...
              <section>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-400 transition-colors">
                  <input
                    ...
                    disabled={isPending}
                  />
                  <label htmlFor="document" className={`cursor-pointer ${isPending ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    {displayFormData.document && (
                      <p className="text-green-600 text-sm mt-2">✓ {displayFormData.document.name}</p>
                    )}
                  </label>
                </div>
              </section>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-end pt-6">
                <button
                  ...
                  disabled={isPending}
                  className="px-8 py-3 border-2 border-red-600 text-red-600 rounded-lg font-medium hover:bg-red-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isPending}
                  className="px-8 py-3 bg-gray-400 text-white rounded-lg font-medium hover:bg-gray-500 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
